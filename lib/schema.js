...
const fields = {
  daysUntilStale: Joi.number().description(
    'Number of days of inactivity before an Issue or Pull Request becomes stale'
  ),

  daysUntilClose: Joi.alternatives()
    .try(Joi.number(), Joi.boolean().only(false))
    .error(() => '"daysUntilClose" must be a number or false')
    .description(
      'Number of days of inactivity before a stale Issue or Pull Request is' +
        ' closed. If disabled, issues still need to be closed manually, but ' +
        'will remain marked as stale.'
    ),

  onlyLabels: Joi.alternatives()
    .try(Joi.any().valid(null), Joi.array().single())
    .description(
      'Only issues or pull requests with all of these labels are checked for' +
        ' staleness. Set to `[]` to disable'
    ),
  ...
  exemptProjects: Joi.boolean().description(
    'Set to true to ignore issues in a project (defaults to false)'
  ),

  exemptMilestones: Joi.boolean().description(
  ...
  staleLabel: Joi.string().description('Label to use when marking as stale'),

  markComment: Joi.alternatives()
    .try(Joi.string(), Joi.any().only(false))
