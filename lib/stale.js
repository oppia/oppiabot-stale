...
module.exports = class Stale {
  ...
  async markAndSweep(type) {
    ...
    const limitPerRun =
      this.getConfigValue(type, 'limitPerRun') || maxActionsPerRun;
  ...
  async sweep(type) {
    ...
    if (daysUntilClose) {
      ...
      this.logger.trace(
        { owner, repo },
        'Configured to leave stale issues open'
      );
    }
  }
  ...
  getStale(type) {
    ...
    const days =
      this.getConfigValue(type, 'days') ||
      this.getConfigValue(type, 'daysUntilStale');
  ...
  getClosable(type) {
    ...
    const days =
      this.getConfigValue(type, 'days') ||
      this.getConfigValue(type, 'daysUntilClose');
    return this.search(type, days, query);
  }
  ...
  async markIssue(type, issue) {
    ...
    var customMarkComment = markComment.replace(
      '@author',
      '@' + pullRequestAuthor
    );
  ...
  async close(type, issue) {
    ...
    if (perform) {
      ...
      this.logger.info(
        '%s/%s#%d would have been closed (dry-run)',
        owner,
        repo,
        number
      );
  ...
  async unmarkIssue(type, issue) {
    ...
    if (perform) {
      ...
      return this.github.issues
        .removeLabel({ owner, repo, number, name: staleLabel })
        .catch((err) => {
  ...
  async ensureStaleLabelExists(type) {
    ...
    return this.github.issues
      .getLabel({ owner, repo, name: staleLabel })
      .catch(() => {
